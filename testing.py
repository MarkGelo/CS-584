...
def train_test_split(X, y):
    ...
    X_train = X[rng][:int((instances * 2) / 3), ]
    X_test = X[rng][int((instances * 2) / 3):, ]
    y_train = y[rng][:int((instances * 2) / 3), ]
    y_test = y[rng][int((instances * 2) / 3):, ]
...
def main():
    ...
    ** Do not modify this function. **
    ...
    X = np.c_[np.ones(X.size), X] # why do they add 1 to each instance?
